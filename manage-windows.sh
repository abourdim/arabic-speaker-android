#!/usr/bin/env bash
# ============================================================================
#  Arabic Speaker Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/app-tier-s-arabic-speaker}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.arabicspeaker}"
AVD_NAME="${AVD_NAME:-arabic_speaker_test}"
...
cmd_test() {
  ...
  local out="/tmp/arabic-speaker-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Arabic Speaker Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
